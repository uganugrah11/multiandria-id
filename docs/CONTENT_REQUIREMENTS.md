...
- `CONTENT NEEDED`: Company profile PDF/deck, if one exists, for a "Download Company Profile"-type asset.
...
- `CONTENT NEEDED`: Does MAI offer CMT (Cut-Make-Trim) and/or FOB (Free on Board) service models? (Both reference companies offer these; not confirmed for MAI.)
...
- `CONTENT NEEDED`: Certifications (ISO, export certifications, etc.), if any — none are currently published anywhere.
